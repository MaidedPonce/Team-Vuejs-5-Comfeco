<template>
  <nav class="flex items-center justify-between shadow-md z-10 flex-wrap px-6 py-3 md:px-10">
    ...
    <div class="block md:hidden">
      <button @click="toggleMenu" class="flex items-center px-3 py-2 border rounded text-gray-800 border-gray-500">
      ...
      <div v-if="isLoggedIn" class="relative">
        <div class="flex items-center">
          ...
          <div @click="toggleDropdown" class="flex items-center cursor-pointer">
            <h3 class="text-gray-600 font-medium mr-2 hidden md:block">{{ user.name }}</h3>
            <img class="w-12 rounded-full hidden md:block" :src="user.photo" :alt="user.name" />
          </div>
        </div>
        <div class="bg-white md:w-40 md:absolute md:right-0 mt-3 md:shadow-md md:rounded-md border-t border-gray-400 md:border-none" :class="{ 'md:hidden': !responsiveDropdown }">
          <h3 href="#" class="text-sm block font-semibold md:px-2 py-2 md:hidden">{{ user.name }}</h3>
          <a href="#" class="w-full flex justify-between items-center md:hidden">
            <p class="text-sm text-pink-600 font-semibold block md:px-2 py-2">Notificaciones</p>
            <span class="text-white bg-purple-500 px-2 rounded-md align-middle items-center">6</span>
          </a>
          <router-link to="/dashboard" class="text-sm block md:px-2 py-2 hover:text-gray-600">Dashboard</router-link>
          ...
          <a @click="logout" href="#" class="text-sm text-red-600 font-bold block md:px-2 py-2 hover:text-red-400">Cerrar Sesion</a>
        </div>
      </div>
      <div v-if="!isLoggedIn">
        <router-link to="/login" class="bg-pink-400 block max-w-min text-sm px-4 py-2 leading-none border rounded text-white border-pink-800 mt-4 md:mt-0 md:inline-block md:mx-1 lg:mt-0">Entrar</router-link>
        <router-link to="/register" class="bg-purple-400 block max-w-min text-sm px-4 py-2 leading-none border rounded text-white border-purple-800 mt-4 md:mt-0 md:inline-block md:mx-1 lg:mt-0">Registrarse</router-link>
      </div>
    </div>
  </nav>
</template>
