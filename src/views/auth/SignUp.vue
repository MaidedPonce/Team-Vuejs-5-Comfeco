  ...
  <div class="flex-grow bg-purple-400">
    <div class="bg-white w-11/12 md:w-3/5 lg:w-2/6 mx-auto my-10 px-6 md:px-10 py-10">
      ...
      <form @submit.prevent="signUp">
        <div>
          ...
          <input
            ...
            :class="{'border-red-400' : ((!$v.nick.required || !$v.nick.minLength || $v.nick.alphaNum) && $v.email.$dirty) }"
            placeholder="Usuario"
          />
          <div>
            <span v-if="!$v.nick.alphaNum && $v.nick.$dirty" class="text-xs text-red-400">Solo puedes ingresar letras y numeros (sin espacios).</span>
            <span v-if="!$v.nick.required && $v.nick.$dirty" class="text-xs text-red-400">Este campo es requerido.</span>
            <span v-if="!$v.nick.minLength && $v.nick.$dirty" class="text-xs text-red-400">Este campo debe tener al menos {{ $v.nick.$params.minLength.min }} caracteres</span>
          </div>
        </div>
        ...
        <div class="my-3">
          ...
          <div>
            ...
            <span v-if="!$v.email.email && $v.email.$dirty" class="text-xs text-red-400">Tienes que ingresar un correo valido</span>
          </div>
        </div>
        <div class="my-3">
          ...
          <input
            ...
            class="w-full text-gray-700 border border-gray-700 focus:outline-none focus:border-pink-700 px-4 py-2 rounded-md"
            :class="{'border-red-400' : ((!$v.password.required || !$v.password.minLength) && $v.password.$dirty) }"
            placeholder="**************"
          />
          <div>
        ...
        <div class="my-3">
          <label for="" class="block text-gray-600 my-1">Confirmar Contrasena</label>
          <input
            name="confirm"
            type="password"
            v-model.trim="$v.confirm.$model"
            class="w-full text-gray-700 border border-gray-700 focus:outline-none focus:border-pink-700 px-4 py-2 rounded-md"
            :class="{'border-red-400' : (!$v.confirm.sameAs && $v.confirm.$dirty) }"
            placeholder="**************"
          />
          <div>
            <span v-if="!$v.confirm.sameAs && $v.confirm.$dirty" class="text-xs text text-red-400">La contrasena debe ser la misma</span>
          </div>
        </div>
        <div class="my-4">
          ...
          <label for="terms">
            Acepto los
            <router-link to="/terminos-y-condiciones" class="text-blue-600 font-semibold" target="_blank">Terminos y condiciones</router-link> y las
            <router-link to="/politicas-de-privacidad" class="text-blue-600 font-semibold" target="_blank">Politicas de privacidad</router-link>
          </label>
          <div class="block">
            <span v-if="!$v.terms.sameAs && $v.terms.$dirty" class="text-xs text-red-400">Debes aceptar los terminos y condiciones y las politicas de privacidad</span>
          </div>
        </div>
      ...
      <p class="text-gray-600 mt-8 text-center">
        Tienes una cuenta?
        <router-link to="/login" class="text-purple-500 font-semibold">Iniciar Sesion</router-link>
      </p>
    </div>
  </div>
</template>
...
<script>
import { required, alphaNum, sameAs, minLength, email } from "vuelidate/lib/validators";
